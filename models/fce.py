...
from data.imca import ContrastiveConditionalDataset, ConditionalDataset, SimpleDataset, ContrastiveSimpleDataset
...
class ebmFCE(object):
    ...
    def train_ebm_fce(self, epochs=500, lr=.0001):
        ...
        # define
        dat_fce = ContrastiveSimpleDataset(np.vstack((self.data, self.noise_samples)).astype(np.float32),
                                           to_one_hot(y)[0].astype(np.float32), device=self.device)
        fce_loader = DataLoader(dat_fce, shuffle=True, batch_size=128)

        # define log normalization constant
        ebm_norm = self.ebm_norm  # -5.
        logNorm = torch.from_numpy(
            np.array(ebm_norm).astype(np.float32))  # , device=dat_fce.device, requires_grad=True )
