...
import os
os.chdir('../../')
from generateToyData import CustomSyntheticDatasetDensity, gen2DgaussMix4comp, gen2Dspiral

# generate data
dataType = 'Spiral'
if dataType == 'MoG':
	data = gen2DgaussMix4comp( n = 2500, scale = .35)
elif dataType == 'Spiral':
	data = gen2Dspiral( n = 2500, radius=3.5) 


dset = CustomSyntheticDatasetDensity( X=data.astype(np.float32), device='cuda:0' ) 
...
convs = [Invertible1x1Conv(dim=2).to(device) for _ in flows]
norms = [ActNorm(dim=2) for _ in flows]
flows = list(itertools.chain(*zip(norms, convs, flows)))
#flowsgpu = [x.to(device) for x in flows ]
...
plt.contour( X, Y, Z / Z.sum() )
plt.title('Flow model log density')
plt.scatter( data[:,0], data[:,1] )
plt.savefig('FlowModelLogDensity_'+dataType+'.png')
