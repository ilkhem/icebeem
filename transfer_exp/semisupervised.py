...
test_loader = DataLoader(test_dataset, batch_size=config.training.batch_size, shuffle=True, num_workers=1, drop_last=True, collate_fn = my_collate )
...
labels -= 8
rep_train, rep_test, lab_train , lab_test = train_test_split( scale(representations), labels, test_size=.25, random_state=0)
...
labels_base -= 8
rep_train_b, rep_test_b, lab_train_b , lab_test_b = train_test_split( scale(representations_base), labels_base, test_size=.25, random_state=0)
